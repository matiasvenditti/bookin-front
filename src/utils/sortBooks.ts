...
export default function(sortBy: SortBy, books: Book[]): Book[] {
    switch (sortBy) {
        ...
        case SortBy.publicationDateAsc: return books.sort((a: Book, b: Book) => new Date(a.date).getTime() - new Date(b.date).getTime());
        case SortBy.publicationDateDes: return books.sort((a: Book, b: Book) => new Date(b.date).getTime() - new Date(a.date).getTime());
